...
func TestOrchestrateWorkersShareReadTs(t *testing.T) {
	runBadgerTest(t, nil, func(t *testing.T, db *DB) {
		...
		require.NoError(t, db.Update(func(txn *Txn) error {
			for i := 0; i < nKeys; i++ {
				if err := txn.Set([]byte(fmt.Sprintf("key-%04d", i)), []byte("v")); err != nil {
					return err
				}
			}
			return nil
		...
		require.NoError(t, db.View(func(txn *Txn) error {
			item, err := txn.Get([]byte("key-0000"))
			if err != nil {
				return err
			}
		...
		go func() {
			...
			for {
				...
				_ = db.Update(func(txn *Txn) error {
					return txn.Set([]byte(fmt.Sprintf("churn-%06d", i)), []byte("x"))
				})
				i++
			}
		}()
		...
		require.Equal(t, 1, len(seen),
			"workers saw %d distinct versions for keys written in one txn; expected 1", len(seen))
		_, ok := seen[initialVersion]
		require.True(t, ok, "expected all items at version %d", initialVersion)
	})
}
